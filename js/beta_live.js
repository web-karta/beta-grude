...
function arrivalsForStation(stationId, tNow) {
  ...
  for (const [key, arr] of tripsByLineDir.entries()) {
    // sortiraj po vremenu polaska
    arr.sort((a, b) => a._t0 - b._t0);

    // 1️⃣ nađi AKTIVAN ili SLJEDEĆI trip
    let tr = null;
...
if (secondsLeft < 0 || secondsLeft > 10 * 60) continue;
...
function makeVehicleIcon(label, angleDeg, showArrow, color) {
  ...
  // Elipsa (badge)
  const cx = 33, cy = 38; // centar elipse
  const rx = 20, ry = 16; // elipsa malo šira (stane P2S)

  // Strelica (offset + veličina)
  const gap = 3;          // koliko je odmaknuta od elipse (povećaj za veći razmak)
  ...
  const svg = `
    <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ${arrow}
      <ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="${C}" stroke="white" stroke-width="2"></ellipse>
